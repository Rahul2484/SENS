...
<p class="fake-legend"><span>LOG TABLE</span>
</p>
...
$sql = "SELECT * FROM `eventcalender` WHERE ID BETWEEN 1 AND 2540000 order by ID desc limit 20";
...
if ($result->num_rows > 0) {
	...
	echo "<body>";
	echo "<form method =get action=ad_delogs.php>";	
	echo "<table cellspacing='0'> <!-- cellspacing='0' is important, must stay -->";
	echo "<tr><th>ID</th><th>Email</th><th>Title</th><th>Events Details</th><th>Event Date</th><th>Date Added</th></tr><!-- Table Header -->";
    while($row = $result->fetch_assoc()) {
        //echo "id: " . $row["id"]. " - Name: " . $row["username"]."    email: " . $row["email"] ;
		//echo "    time is :" . $row["time"]. "    event details are : " .$row["edetail"]."<br>";
		//echo "$row[eventDate]";
		
		echo "<tr><td><input type=checkbox name=check_list[] value=$row[ID]> $row[ID] </td><td>$row[Email]</td><td>$row[Title]</td><td>$row[Detail]</td><td>$row[eventDate]</td><td>$row[dateAdded]</td></tr><!-- Table Row -->";
